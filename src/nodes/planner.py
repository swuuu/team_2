    ...
    def map_callback(self, msg):
        ...
        # for obs in ['/obstacle1']:
            try:
                ...
                # Move sideways
                # alignment_diff = np.abs(np.arctan2(delta_Y, delta_X) - np.arctan2(obs_Y, obs_X))
                # rospy.logerr("2")
                
                # if alignment_diff > 0.2:
                #     if delta_Y > 0:
                #         vel_X = vel_X + self.k_side * vel_Y
                #         vel_Y = vel_Y - self.k_side * vel_X
                #     else:
                #         vel_X = vel_X - self.k_side * vel_Y
                #         vel_Y = vel_Y + self.k_side * vel_X
        ...
        self.cmd.linear.x = vel_des[0]
        self.cmd.linear.y = vel_des[1]
